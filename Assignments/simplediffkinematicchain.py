...
class DiffKinematicChain(kc.KinematicChain):
    ...
    def Jacobian_Ad_inv(
        ...
        self.link_positions_with_base = deepcopy(self.link_positions)
        ...
        for j in range(1, link_index + 1):
            ...
            g_rel: rplg.RepresentationLieGroupElement = (
                self.link_positions_with_base[j - 1].inverse * selected_link
            )
            ...
            axis_at_link_end = g_rel.Ad_inv(self.joint_axes[j - 1])
            ...
            if output_frame == "world":
                result = selected_link.TL(axis_at_link_end)
            ...
            #! body to spatial conversion at link
                ...
                result = selected_link.Ad(axis_at_link_end)
            else:
                raise ValueError(
                    f"Only spatial and world frame supported! you gave {output_frame}!"
                )

            # Insert the value of J_joint into the (j-1)th index of J
            J[j - 1] = np.asarray(result.value)
    ...
    def Jacobian_Ad(
        ...
        for j in range(1, link_index + 1):
            ...
            spatial_axis = selected_link.AD(self.joint_axes[j - 1])

            # If the output_frame input is 'world', map the axis from the Lie algebra out to world coordinates
            if output_frame == "world":
                result = selected_link.TL(spatial_axis)
            ...
            # the identity
                ...
                result = selected_link.Ad_inv(spatial_axis)
            else:
                raise ValueError(
                    f"Only body and world frame supported! you gave {output_frame}!"
                )
    ...
    def draw_Jacobian(self, ax: plt.Axes):
        ...
        x_spacing = 2
        y_spacing = 2
        ax.plot([0, 0], [min(min_config_y - y_spacing, 0), max_config_y + y_spacing], '--k', alpha=0.0)
        ax.plot([min(min_config_x - x_spacing, 0), max_config_x + x_spacing], [0, 0], '--k', alpha=0.0)
        ax.quiver(x_configs, y_configs, j_x, j_y, angles='xy', scale_units='xy', scale = 4.0, width = 0.007, color="red")
        ax.set_aspect('equal')
...
if __name__ == "__main__":
    ...
    config3 = [0.25 * np.pi, -0.5 * np.pi, 0.5 * np.pi]
    ...
    for i, ax_config in enumerate(ax[0]):
        ...
        ax_config: plt.Axes
        ax_config.set_title(f"Jacobian Ad_inv at {np.round(configs[i], 2)}")
    for i, ax_config in enumerate(ax[1]):
        ...
        for j in range(1, len(links) + 1):
            ...
            kc.draw_Jacobian(ax_config)
        ax_config: plt.Axes
        ax_config.set_title(f"Jacobian Ad at {np.round(configs[i], 2)}")
