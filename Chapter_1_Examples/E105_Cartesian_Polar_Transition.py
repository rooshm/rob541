...
from S100_Construct_R2 import R2  # Get the R2 manifold constructed previously
...
q = Q.element([3, 3], 0)
print("Initial configuration in Cartesian coordinates is ", q)

# Convert the point to polar coordinates
q_polar = q.transition(1)
print("Configuration in polar coordinates ", q_polar)
...
ax_orig.scatter(q[0], q[1], color=spot_color)
...
ax_polar.scatter(q_polar[1], q_polar[0], color=spot_color)
...
ax_returned.scatter(q_cartesian[0], q_cartesian[1], color=spot_color)
