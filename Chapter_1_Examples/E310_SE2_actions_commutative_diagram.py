...
ax.set_xlim(-1, 4)
ax.set_ylim(-4, 3)
circle = plt.Circle([0, 0], np.sqrt(g_initial[0] * g_initial[0] + g_initial[1] * g_initial[1]), edgecolor='black',
                    facecolor='none', linestyle='dashed')
ax.add_artist(circle)
circle = plt.Circle([0, 0], np.sqrt(gi_h[0] * gi_h[0] + gi_h[1] * gi_h[1]), edgecolor='black', facecolor='none',
                    linestyle='dashed')
ax.add_artist(circle)
ax.set_aspect('equal')
...
circle = plt.Circle([0, 0], np.sqrt(gi_h[0] * gi_h[0] + gi_h[1] * gi_h[1]), edgecolor='black', facecolor='none',
                    linestyle='dashed')
ax.add_artist(circle)
ax.arrow(gd_theta_gi[0], gd_theta_gi[1], .9 * (gd_gi[0] - gd_theta_gi[0]), .9 * (gd_gi[1] - gd_theta_gi[1]), zorder=0,
         length_includes_head=True, width=.01, head_width=0.2, overhang=.5, facecolor='black')
ax.arrow(gd_theta_gi_h[0], gd_theta_gi_h[1], .9 * (gd_gi_h[0] - gd_theta_gi_h[0]), .9 * (gd_gi_h[1] - gd_theta_gi_h[1]),
         zorder=0, length_includes_head=True, width=.01, head_width=0.2, overhang=.5, color='black')
ax.arrow(g_initial[0], g_initial[1], .9 * (gi_h[0] - g_initial[0]), .9 * (gi_h[1] - g_initial[1]), zorder=0,
         length_includes_head=True, width=.01, head_width=0.2, overhang=.5, color=spot_color)
ax.arrow(gd_gi[0], gd_gi[1], .9 * (gd_gi_h[0] - gd_gi[0]), .9 * (gd_gi_h[1] - gd_gi[1]), zorder=0,
         length_includes_head=True, width=.01, head_width=0.2, overhang=.5, color=spot_color)
