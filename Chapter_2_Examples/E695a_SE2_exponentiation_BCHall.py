...
triangles_ApB.append(RigidBody(cornered_triangle(.1, spot_color), g_circ_B.exp_R * g_circ_A.exp_R))
...
###
# get the A-only flow, starting at the rotated origin
g_circ = g_circ_A
start1 = 0
end1 = 1
g_initial = g_circ_B.exp_R

# a = G.L_generator([1, 0, 0])
# b = a(G.element([0, 0, 1.5]))
sol = G.R_generator(g_circ.value).integrate([start1, end1], g_initial)
...
triangles_ApBpLAB.append(RigidBody(cornered_triangle(.1, spot_color), g_circ_B.exp_R * g_circ_A.exp_R))
...
for t in triangles_ApB:
    t.draw(ax)
ax.plot(*traj_B[0:2], color='black', zorder=-3) #, marker='.', markersize=10)
